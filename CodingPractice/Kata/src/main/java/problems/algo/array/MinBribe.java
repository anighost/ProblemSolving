...
public class MinBribe {
	...
	static void minimumBribes(int[] q) {
		...
		Map<Integer, Integer> bribeMap = new HashMap<Integer, Integer>();
		...
		for (int i = 0; i < q.length; i++) {
			for (int j = 0; j < q.length - i - 1; j++) {
				if (q[j] > q[j+1]) {
					//swap and keep count
					int count = 1;
					if(bribeMap.get(q[j]) != null) {
						count = bribeMap.get(q[j]) + count;
						if(count > 2) {
		...
		int bribeCount = 0;
		for(Entry<Integer, Integer> entry : bribeMap.entrySet()) {
			bribeCount = bribeCount + entry.getValue();
		}
	...
	public static void main(String[] args) {
		...
		int[] a1 = new int [] {2, 1, 5, 3, 4};
		//begin - 1,2,3,4,5
		//current - 2,1,5,3,4
		
		minimumBribes(a1); //3

		int[] a2 = new int [] {2, 1, 5, 4, 3};
		minimumBribes(a2); //4
